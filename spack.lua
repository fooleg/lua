...
function spack.decode(format, stream)
  local vars = {}
  local iterator = 1
  local endianness = true
...
SPackWriter = {}
SPackWriter.__index = SPackWriter

function SPackWriter:new(ender)
  local self = setmetatable({}, SPackWriter)
  self.buffer = {}
  self.bl_end = ender or '>'
  return self
end
function SPackWriter:write(type, v)
  b = spack.encode(self.bl_end..type, v)
  table.insert(self.buffer, b)
end
function SPackWriter:Byte(v)
  self:write('b', v)
end
function SPackWriter:Short(v)
  self:write('h', v)
end
function SPackWriter:Int(v)
  self:write('i', v)
end
